...
namespace Ajuda.Commands
{
    class AjudaComando : IRocketCommand
    {
        ...
        public List<string> Permissions => new List<string> { "HelpSystem.Help"};
        ...
        public void Execute(IRocketPlayer caller, string[] command)
        {
            ...
            {
                Text = "";
            for(int timer = 1; timer < command.Length-1; timer++)
                {
                    Text = Text + " " + command[timer];
                }
            }
            else
            {
            ...
            Plugin.Instance.Helps.Add(new Models.HelpSystem(unturnedPlayer, command[0], Text, a));
